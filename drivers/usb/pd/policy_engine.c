...
int usbpd_get_plug_status(struct usbpd *pd)
{
	int ret;
...
static void handle_vdm_rx(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	usbpd_dbg(&pd->dev, "VDM rx: svid:%x cmd:%x cmd_type:%x vdm_hdr:%x has_dp: %s\n", 
		svid, cmd, cmd_type, vdm_hdr, pd->has_dp ? "true" : "false");
...
static void usbpd_sm(struct work_struct *w)
{
	...
	if (pd->current_pr == PR_NONE) {
		if (pd->current_state == PE_UNKNOWN)
			#ifdef VENDOR_EDIT
			/* tongfeng.Huang@BSP.CHG.Basic, 2018/09/07,  Add for otg host does not close after cable plugout in some case */
			if(pd->current_dr == DR_NONE)
			#endif
				goto sm_done;
